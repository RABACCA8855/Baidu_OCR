...
def main():
    ...
    if table_file:
        ...
        if request_id not in ['over limit', 'unkown msg']:
            ...
            if request_id:
                ...
                print("final download:", download_url)
                if download_url:
                    st.markdown('点击以下网址,获取表单excel')
                    if download_url == '图片识别异常':
                        st.error('验证码获取成功，但图片识别异常')
                    elif download_url == '进行中':
                        st.warning('图片识别进行中，请稍后!')
                        sleep(5)
                        download_url = get_img(request_id)
                    ...
                    st.error('验证码获取成功，但是无法获取下载地址！')
            else:
                st.error('未能成功获取验证码，如有紧急需求请联系病理大数据部魏博、骆佳俊！')
        elif request_id == 'over limit':
            st.warning('当天使用该功能超过五十次，请明天再试！')
        else:
            st.error('无法获取验证码，请联系算法工程师解决！')
